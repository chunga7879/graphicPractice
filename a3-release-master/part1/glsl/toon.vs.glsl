...
void main() {
    ...
    // because Silhouettes  are characterized by normal vectors
    // that are parallel to the viewing plane and therefore orthogonal to the direction to the viewer.
    // orthogonal btw viewDirection and normal => Silhouettes
    fresnel = dot(viewDirection, viewNormal); // because length is 1


    // Multiply each vertex by thes model matrix to get the world position of each vertex,
